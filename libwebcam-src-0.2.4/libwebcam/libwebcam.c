	...
	switch(error) {
		...
		case C_V4L2_ERROR:
		{
			...
			if(hDevice && HANDLE_OPEN(hDevice)) {
				sprintf(text, "A Video4Linux2 API call returned an unexpected error %d", GET_HANDLE(hDevice).last_system_error);
			}
			if(!text)
...
void print_libwebcam_error (char *format, ...)
{
	char *newformat=calloc(1, 1024);
	va_list ap;

	if(sprintf(newformat, "[libwebcam] %s\n", format) == -1)
		newformat = format;
...
static void print_libwebcam_c_error (CResult error, char *format, ...)
{
	...
	char *text, *newformat=calloc(1, 1024);
	va_list ap;
	...
	if(sprintf(newformat, "[libwebcam] %s (error %d: %s)\n", format, error, text) == -1)
		newformat = format;
...
static Control *create_v4l2_control (Device *device, struct v4l2_queryctrl *v4l2_ctrl, int v4l2_dev, CResult *pret)
{
	...
	switch(v4l2_ctrl->type) {
		...
		case V4L2_CTRL_TYPE_CTRL_CLASS:		type = CC_TYPE_CLASS;		break;
...
static CControlId get_control_id_from_v4l2 (int v4l2_id, Device *dev)
{
	...
	if ((strcmp(dev->device.driver, "uvcvideo") == 0) || (strcmp(dev->device.driver, "devu-uvc") == 0)) {
		switch(v4l2_id) {
	...
	if (V4L2_CTRL_ID2CLASS(v4l2_id) == V4L2_CTRL_CLASS_USER) {
		if (v4l2_id==V4L2_CID_USER_CLASS) {
			return 0;
		}
		...
		if (v4l2_id==V4L2_CID_MPEG_CLASS) {
			return 0;
		}
		...
		if (v4l2_id==V4L2_CID_CAMERA_CLASS) {
			return 0;
		}
...
static CResult get_device_usb_info (Device *device, CUSBInfo *usbinfo)
{
	...
	for(i = 0; i < 3; i++) {
		...
		if(sprintf(filename, "/sys/class/video4linux/%s/device/../%s",
